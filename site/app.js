...
function annotateDailySummaryWithReferences(text, refEntries = []) {
  ...
  refEntries.forEach((entry) => {
    ...
    const citation = canonical ? `[${title}](#paper-${canonical})` : title;
    output = output.replace(pattern, citation);
  });
  // Clean possible duplicate numbering artifacts generated by model.
  output = output
    .replace(/[“"]{2,}/g, "“")
    .replace(/([”"])\s*\d+\s*([”"])/g, "$1")
    .replace(/([”"])\s*\d+\s*(?=\[)/g, "$1")
...
async function triggerSummaryDailyViaWorker() {
  ...
  try {
    ...
    if (!refEntries.length) {
      setSummaryLoading(false, "", { conversationId: taskConversationId });
      pushSummaryDialogMessage("system", "最近1天论文列表为空，无法生成日报。", { conversationId: taskConversationId });
      setSummaryMessage("日报生成失败：未获取到论文列表。");
      return;
    }
